# R functions to run a decision forest (ensemble of decision trees), as 
#  directed by the RMPI framework for Insilicos Ensemble Cloud Army
...
common_preamble <- function() {
    ...
    nClasses <<- length( unique( train[ , nTrainCols ] ) )
...
client_ensemblecalc <- function( randSeedVal )
{
    ...
    featureSubset <- c( sample( 1 : ( nTrainCols - 1 ), nFeatSub ), nTrainCols )
	...
	form <- as.formula( paste( names( train )[ nTrainCols ], ".", sep = "~", collapse = "" ) )
	...
	returnval <- predict( fit, newdata = testSub, type = "class" ) # return value is testClassPred, a "factor of classifications" per http://cran.r-project.org/web/packages/rpart/rpart.pdf
}
...
head_resulthandler <- function(result, ensembleID) {
	...
	eca_log( paste( "tree ", ensembleID, " : ", nCorrTestPred, "/", nTestRows, " correct", sprintf( " ( %5.2f%% )", pctCorrTestPred ), sep = "" ), timeStamp = FALSE )
...
head_postscript <- function()
{
	...
	counts <- array( rep( 0, nTestRows * nClasses ), c( nTestRows, nClasses ) )
	for ( samp in 1 : nTestRows )
	{
		for ( cla in 1 : nClasses )
		{
    ...
    eca_log( paste( "individual tree average : ", sprintf( "%8.2f +/- %8.2f / %d ( %5.2f +/- %5.2f %% )", meanNCorrTestPred, stdNCorrTestPred, nTestRows, pctMeanNCorrTestPred, pctStdNCorrTestPred ), sep = "" ), timeStamp = FALSE )
	nCorrEnsemPred <- sum( testClassTrue == testClassPredMax )
	pctCorrEnsemPred <- 100.0 * nCorrEnsemPred / nTestRows
    eca_log( paste( "ensemble of trees       : ", nCorrEnsemPred, " / ", nTestRows, " correct", sprintf( " ( %5.2f%% )", pctCorrEnsemPred ), sep="" ), timeStamp = FALSE )
    eca_log( " ", timeStamp = FALSE )
}
