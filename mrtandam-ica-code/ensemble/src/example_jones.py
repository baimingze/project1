...
while True :
	...
	if ("1"==mode) :
		...
		commandline = 'eca_launch_rmpi.py general_config.json decision_tree_jones_job_config.json --local'
	elif ("2"==mode) :
		print "using RMPI on Amazon EC2"
		commandline = 'eca_launch_rmpi.py general_config.json decision_tree_jones_job_config.json'
	elif ("3"==mode) :
		print "using MapReduce on Amazon EMR"
		commandline = 'eca_launch_mapreduce.py general_config.json decision_tree_jones_job_config.json'
	elif ("q"==mode) :
		exit(0)
	if ("" != commandline) :
		import platform
